...
namespace
{
    ...
    DBRow GetRow(sqlite3_stmt* stmt, int columnCount)
    {
        ...
        for (int columnIdx = 0; columnIdx < columnCount; ++columnIdx) {
            ...
            auto name = s3d::Unicode::Widen(nameRaw);
            ...
            case SQLITE_TEXT:
            {
                ...
                record.emplace(name, s3d::Unicode::Widen(reinterpret_cast<const char*>(value)));
            }
            break;
            case SQLITE_BLOB:
            {
                ...
                record.emplace(name, s3d::Blob(value, size));
            }
            break;
            default:
                record.emplace(name, nullptr);
...
namespace s3dsql
{
    ...
    bool SQLite3Stmt::prepare(s3d::StringView sql)
    {
        ...
        if (result != SQLITE_OK || !m_stmt) {
            throw SQLError(U"Failed SQL : {}"_fmt(s3d::Unicode::Widen(sqlite3_errmsg(m_db))));
        }
        return true;
    }
    ...
    bool SQLite3Stmt::bind(s3d::int32 index, const DBValue& value) const
    {
        ...
        case DBValueType::String:
            result = sqlite3_bind_text(m_stmt, index, s3d::Unicode::ToUTF8(value.get<s3d::String>()).c_str(), -1, SQLITE_TRANSIENT);
            break;
